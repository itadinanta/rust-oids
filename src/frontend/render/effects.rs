...
impl<R: gfx::Resources, C: gfx::CommandBuffer<R>> PostLighting<R, C> {
	...
	where F: gfx::Factory<R> {
		let full_screen_triangle = vec![
			...
			BlitVertex { pos: [3., -1.], tex_coord: [2., 0.] },
		];
	...
	)
	{
		...
		self.full_screen_pass(encoder, &self.resolve_msaa_pso, raw_hdr_src, &self.resolved.2);
		...
		encoder.update_constant_buffer(&self.smooth_fragment_args, &SmoothFragmentArgs { exp_alpha: 0.05 });
		encoder.draw(&self.index_buffer_slice, &self.smooth_pso, &smooth::Data {
			vbuf: self.vertex_buffer.clone(),
		...
		// Tone mapping
		encoder.update_constant_buffer(&self.tone_map_vertex_args, &ToneMapVertexArgs { white: 4.0, black: 0.5 });
		encoder.draw(&self.index_buffer_slice, &self.tone_map_pso, &tone_map::Data {
			vbuf: self.vertex_buffer.clone(),
		...
		self.full_screen_pass(encoder, &self.blit_pso, &self.luminance_smooth.1, &self.luminance_acc.2);
		// Bloom
		// 1. extract high luminance
		self.full_screen_pass(encoder, &self.highlight_pso, &ping_pong_full[0].1, &ping_pong_half[0].2);
		// 2. horizontal 4x, 9x9 gaussian blur
		self.full_screen_pass(encoder, &self.blur_h_pso, &ping_pong_half[0].1, &ping_pong_half[1].2);
		// 2. vertical 4x, 9x9 gaussian blur
		self.full_screen_pass(encoder, &self.blur_v_pso, &ping_pong_half[1].1, &ping_pong_half[0].2);

		// compose tone mapped + bloom and resolve
		encoder.draw(&self.index_buffer_slice, &self.compose_pso, &compose::Data {
			vbuf: self.vertex_buffer.clone(),
			// original
