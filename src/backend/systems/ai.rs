...
impl AiSystem {
    fn update_minions(targets: &IdPositionMap, beacons: &[Position], minions: &mut agent::AgentMap) {
        fn nearest_beacon<'a>(beacons: &'a [Position], p: &'a Position) -> &'a Position {
            beacons.iter()
                .fold1(|n, b| if (p - n).magnitude2() < (p - b).magnitude2() { n } else { b })
                .unwrap_or(p)
        }
                ...
                for segment in segments.iter_mut() {
                    ...
                    if flags.contains(segment::Flags::ACTUATOR) {
                        ...
                        let intent =
                            ...
                            } else if flags.contains(segment::Flags::RUDDER | segment::Flags::LEFT) && r[0] > brain.hunger() {
                                Intent::Move(-f)
                            } else if flags.contains(segment::Flags::RUDDER | segment::Flags::RIGHT) && r[1] > brain.hunger() {
                                Intent::Move(-f)
