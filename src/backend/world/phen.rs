...
impl Phenotype for Minion {
    fn develop(gen: &mut Genome, id: Id, transform: &Transform, motion: Option<&Motion>, charge: f32) -> agent::Agent {
        ...
        builder.addr(torso, i, &gen.star(), Flags::ARM | Flags::JOINT | Flags::ACTUATOR | Flags::RUDDER)
            .addl(torso, -i, &gen.star(), Flags::ARM | Flags::JOINT | Flags::ACTUATOR | Flags::RUDDER);

        let head = builder.add(torso, 0, &head_shape, Flags::HEAD | Flags::MOUTH | Flags::SENSOR | Flags::TRACKER).index();
        builder.addr(head, 1, &gen.triangle(), Flags::HEAD | Flags::ACTUATOR | Flags::RUDDER)
            .addl(head, -1, &gen.triangle(), Flags::HEAD | Flags::ACTUATOR | Flags::RUDDER);
        ...
        builder.addr(belly, belly_mid - 1, &leg_shape,  Flags::LEG |  Flags::ACTUATOR |  Flags::THRUSTER)
