...
impl From<Emitter> for Message {
	fn from(value: Emitter) -> Self { Message::NewEmitter(value) }
}

impl From<Event> for Message {
	fn from(value: Event) -> Self { Message::Event(value) }
}

impl From<Alert> for Message {
	fn from(value: Alert) -> Self { Message::Alert(value) }
}
...
where M: Send + Clone
{
	pub fn new() -> Self { PubSub { filters: Vec::new() } }
}
...
impl<M> ReceiveDrain<M> for Inbox<M>
where M: Send + Clone
{
	fn drain(&self) -> Vec<M> {
		let mut acc = Vec::new();
...
pub trait DrainInto<M, T>: ReceiveDrain<M>
where M: Send + Clone + Into<Option<T>> {
	fn drain_into(&self) -> Vec<T> { self.drain().into_iter().map(Into::into).filter_map(|i| i).collect::<Vec<T>>() }
}
